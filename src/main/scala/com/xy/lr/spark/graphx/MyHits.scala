...
object MyHits{
  ...
  def run(graph : Graph[String, String], maxIterations: Int = Int.MaxValue, saveFilePath : String
           ): Graph[(Double, String), String] ={

    //initial the graph by set first 1.0 value
    @transient var initialGraph : Graph[(Double/*hits value*/ , String), String] = graph.mapVertices(
      (id, attr) =>{
        (1.0, attr)
      }
    )
    ...
    while(t < maxIterations){
      ...
      t = t + 1
      initialGraph = hitsGraph

      hitsGraph.vertices.map( x => {
        (x._1, x._2._1)
      }).saveAsTextFile(saveFilePath + t)
      hitsGraph
    }
